...
public class GameManager : MonoBehaviour
{
    ...
    [SerializeField] private GameObject wp; // wayPoint prefab
    ...
    /// <summary>
    /// Method SetMaxScore
    /// This method updates the max score registered 
    /// </summary>
    ...
    /// This method build a gameObject image list of lives to prevent an expensive GetComponent call
    /// </summary>
    ...
    /// <summary>
    /// Method BuildWayPoints
    /// </summary>
    private void BuildWayPoints()
    {
        Transform wpContainer = GameObject.Find("wayPointContainer").transform;
        
        // Get tileMap limits.
        BoundsInt bounds = tilemap[3].cellBounds;
        int cellCounter = 0;
        for (int y = bounds.yMax; y >= bounds.yMin; y--)
        {
            for (int x = bounds.xMin; x < bounds.xMax; x++) // 
            {
                ...
                // Test wp map
                if (tilemap[3].GetTile(cell) != null)
                {
                    var v = tilemap[3].CellToWorld(cell); 
                    var pos = (v += tilemap[3].cellSize / 2);
                    var wpObj = Instantiate(wp,new Vector3(v.x,v.y,-1) , Quaternion.identity); // Create a prefab wayPoint
                    wpObj.name = $"wp_{cellCounter}";
                    wpObj.transform.SetParent(wpContainer);
                    
                    // Building a WayPoint list property
